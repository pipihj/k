...
object implementation {
  ...
  private object ConcreteClasses {
    ...
    case class Definition(att: i.Attributes, modules: immutable.Seq[i.Module]) extends i.Definition

    case class Module(name: String, decls: immutable.Seq[i.Declaration], att: i.Attributes)
        extends i.Module

    case class Import(name: String, att: i.Attributes) extends i.Import

    case class SortDeclaration(
        params: immutable.Seq[i.SortVariable],
        sort: i.Sort,
        att: i.Attributes
    ...
    case class Attributes(patterns: immutable.Seq[i.Pattern]) extends i.Attributes
    ...
    case class Application(head: i.SymbolOrAlias, args: immutable.Seq[i.Pattern])
        extends i.Application

    case class Top(s: i.Sort) extends i.Top

    case class Bottom(s: i.Sort) extends i.Bottom

    case class And(s: i.Sort, args: immutable.Seq[i.Pattern]) extends i.And

    case class Or(s: i.Sort, args: immutable.Seq[i.Pattern]) extends i.Or
    ...
    case class CompoundSort(ctr: String, params: immutable.Seq[i.Sort]) extends i.CompoundSort {
      override lazy val toString      = ctr + "{" + params.map(_.toString).mkString(", ") + "}"
      override lazy val hashCode: Int = scala.runtime.ScalaRunTime._hashCode(this)
    }

    case class SymbolOrAlias(ctr: String, params: immutable.Seq[i.Sort]) extends i.SymbolOrAlias {
      override lazy val toString      = ctr + "{" + params.map(_.toString).mkString(", ") + "}"
      override lazy val hashCode: Int = scala.runtime.ScalaRunTime._hashCode(this)
    }

    case class Symbol(ctr: String, params: immutable.Seq[i.Sort]) extends i.Symbol

    case class Alias(ctr: String, params: immutable.Seq[i.Sort]) extends i.Alias
  }
  ...
  object DefaultBuilders extends i.Builders {
    ...
    def Definition(att: i.Attributes, modules: immutable.Seq[i.Module]): i.Definition =
      d.Definition(att, modules)

    def Module(name: String, decls: immutable.Seq[i.Declaration], att: i.Attributes): i.Module =
    ...
    def Attributes(patterns: immutable.Seq[Pattern]): i.Attributes = d.Attributes(patterns)
    ...
    def Application(head: i.SymbolOrAlias, args: immutable.Seq[i.Pattern]): i.Pattern =
    ...
    def And(s: i.Sort, _1: i.Pattern, _2: i.Pattern): i.Pattern = d.And(s, immutable.Seq(_1, _2))

    def And(s: i.Sort, args: immutable.Seq[i.Pattern]): i.Pattern =
      args.size match {
        case 0 => Top(s)
        case 1 => args(0)
        case _ => d.And(s, args)
    ...
    def Or(s: i.Sort, _1: i.Pattern, _2: i.Pattern): i.Pattern = d.Or(s, immutable.Seq(_1, _2))

    def Or(s: i.Sort, args: immutable.Seq[i.Pattern]): i.Pattern =
      args.size match {
        case 0 => Bottom(s)
        case 1 => args(0)
        case _ => d.Or(s, args)
    ...
    def CompoundSort(ctr: String, params: immutable.Seq[i.Sort]): i.CompoundSort =
      d.CompoundSort(ctr, params)

    def SymbolOrAlias(ctr: String, params: immutable.Seq[i.Sort]): i.SymbolOrAlias =
      d.SymbolOrAlias(ctr, params)

    def Symbol(ctr: String, params: immutable.Seq[i.Sort]): i.Symbol = d.Symbol(ctr, params)

    def Alias(ctr: String, params: immutable.Seq[i.Sort]): i.Alias = d.Alias(ctr, params)

    def LeftAssoc(
        ctr: (i.Pattern, i.Pattern) => i.Pattern,
        args: immutable.Seq[i.Pattern]
    ): i.Pattern =
